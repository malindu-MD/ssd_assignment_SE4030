...
router.delete("/delete/:id", (req, res) => {
  inquiry.findByIdAndRemove(req.params.id).exec((err, deletedPost) => {
    if (err)
      return res.status(400).json({
        message: "Inquiry Delete unsuccesful",
        err,
      });
    return res.json({
      message: "Inquiry Delete succesful",
      deletedPost,
    });
  });
});
...
router.route("/view2").post((req, res) => {
  const nic = req.body.nic;
  ...
  inquiry.findOne({ nic: nic, userId: userId }).then((post) => {
    if (post == null) {
      res.json("No Inquiry Found");
    } else {
      res.json({
        id: post._id,
        name: post.name,
